"""
102TH STUDY: Auto-Parse Test v2
================================
...
TITLE = "102TH Study: Solo Melody Test (eightyth setup)"
...
#
# ⚠️  CRITICAL: SNIPPET NAMING CONVENTION ⚠️
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 
# **UPPERCASE Names (e.g., THEME_A, INTRO_MELODY, BASS_FIGURE)**
#   → INPUT snippets (Station 1)
#   → These are YOUR compositions
#   → The framework WILL parse these from VOICE_STAVE_DATA
#   → Use these in your blueprint
#
# **lowercase names (e.g., transpose_part_THEME_A_P5, retrograde_part_INTRO)**
#   → OUTPUT snippets (Station 2 - generated automatically)
#   → These are TRANSFORMATIONS created by the system
#   → The framework will INTENTIONALLY IGNORE these in blueprint parsing
#   → Cannot be used directly in VOICE_STAVE_DATA
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 
# THE "TOGGLE" MECHANISM (Controls Which Station is Leading):
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 
# Parser pattern: r'^[A-Z_][A-Z0-9_]*$'  ← Only matches UPPERCASE
#
# 🔴 TOGGLE OFF (Default - Station 1 Leading):
#   ✅ UPPERCASE names → Parser reads from Station 1
#   ❌ lowercase names → Parser ignores (Station 2 output)
#   Result: Fresh transformations each run
#
# 🟢 TOGGLE ON (Station 2 Leading - Manual):
#   To use Station 2 snippets: Copy & rename to UPPERCASE
#   ✅ Renamed UPPERCASE → Parser reads as Station 1 input
#   Result: Station 2 becomes new Station 1
#
# Workflow (Toggle OFF - Default):
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 
# Study 1 (studies/102th.py):
#   THEME_A = r"""c4 d4 e4 f4"""  ← Station 1 input
#   VOICE_STAVE_DATA = """transpose_part(THEME_A, 'P5')"""
#   → outputs/102th.py generates:
#       transpose_part_THEME_A_P5 = r"""g4 a4 b4 c'4"""  ← Station 2 (lowercase)
#
# Study 2 (studies/103th.py) - Toggle remains OFF:
#   TRANSPOSED_THEME = r"""g4 a4 b4 c'4"""  ← Copy + RENAME (toggle ON for this snippet)
#   VOICE_STAVE_DATA = """retrograde_part(TRANSPOSED_THEME)"""
#   → Station 2 snippet becomes Station 1 input (conscious choice)
#
# ❌ WRONG (toggle stays OFF, lowercase ignored):
#   VOICE_STAVE_DATA = """transpose_part_THEME_A_P5"""  ← Parser ignores!
...
# - Always use UPPERCASE for snippet variable names
...
\relative c'' {
    ...
    \time 3/4
    \tempo "Andante" 4=90
    d4-.\p( f8 g) a4~ |      % Staccato, piano, slur, tie
    a4 g4->( f) |             % Accent, phrase
...
\relative c'' {
    ...
    \time 3/4
    \tempo "Andante" 4=90
    d4-.\f( g8 g) a4~ |      % Staccato, forte, slur, tie
    a4 g4->( fis) |             % Accent, phrase
    e4.( d8~ d4) |              % Dotted rhythm
    b'4\ff c4 d4                % Fortissimo, climax
}
""".strip()
...
# ============================================================================
# STATION 2: SNIPPET LIBRARY (Automatic Documentation Output)
# ============================================================================
#
# Station 2 appears in the OUTPUT file (outputs/102th.py) AFTER pipeline runs.
# You DON'T write code here - the framework generates it automatically!
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# WHAT STATION 2 CONTAINS (in outputs/102th.py):
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# 1. ORIGINAL SNIPPETS (from Station 1):
#    THEME_A = r"""d'''4 e'''4 f'''4 g'''4"""  ← Absolute notation
#
# 2. TRANSFORMED SNIPPETS (from Station 3 transformations):
#    transpose_part_THEME_A_P5 = r"""a'''4 b'''4..."""  ← lowercase output!
#    retrograde_part_THEME_A = r"""g'''4 f'''4..."""
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# THE "TOGGLE" MECHANISM:
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# Parser pattern: r'^[A-Z_][A-Z0-9_]*$'  ← UPPERCASE ONLY
#
# 🔴 Toggle OFF (Default):
#   ✅ THEME_A, INTRO_MELODY           → Parser reads (Station 1)
#   ❌ transpose_part_THEME_A_P5       → Parser ignores (Station 2 output)
#   ❌ retrograde_part_INTRO           → Parser ignores (Station 2 output)
#   Result: Station 1 is LEADING
#
# 🟢 Toggle ON (for specific snippet):
#   Copy Station 2 snippet → Rename to UPPERCASE → Now becomes Station 1 input
#   Example: transpose_part_THEME_A_P5 → TRANSPOSED_THEME
#   Result: Station 2 snippet becomes LEADING (conscious choice)
#
# Why lowercase in Station 2?
#   - Prevents accidental reuse of stale cached output
#   - Forces conscious decision to "lock in" transformations
#   - Ensures reproducible builds (Station 1 = explicit input)
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# INTERNAL LIBRARY (During Execution):
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# Behind the scenes, the framework maintains SNIPPETS dictionary:
#   SNIPPETS['INTRO'] = [event1, event2, ...]       # Parsed from Station 1
#   SNIPPETS['THEME_A'] = [...]                      # Parsed from Station 1
#   SNIPPETS['transpose_part(THEME_A)'] = [...]     # Generated in Station 3
#
# Console output shows parsing progress:
#   ✓ INTRO: 12 events
#   ✓ THEME_A: 8 events
#   ✓ BASS: 10 events
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# HOW TO USE STATION 2 OUTPUT (Toggle ON - Make Station 2 Leading):
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# Default: Toggle OFF (Station 1 leading, lowercase ignored)
# To make Station 2 leading: Copy + Rename to UPPERCASE
#
# 1. Run this study → check outputs/102th.py for Station 2 snippets
# 2. Copy snippet: transpose_part_THEME_A_P5 = r"""a'''4 b'''4..."""
# 3. Create new study (103th.py)
# 4. Paste in Station 1 with UPPERCASE name:
#    TRANSPOSED_THEME = r"""a'''4 b'''4..."""  ← Toggle ON for this snippet!
# 5. Use in blueprint:
#    VOICE_STAVE_DATA = """retrograde_part(TRANSPOSED_THEME)"""
#
# Result: Second-generation transformation (Station 2 → Station 1)
...
# ----------------------------------------------------------------------------
# --- Variant 1: Single Staff, Single Voice (Solo) ⭐ ACTIVE EXAMPLE
# ----------------------------------------------------------------------------
# Use for: Solo pieces, single melodic lines
...
def build_score_data() -> Dict:
    ...
    print("\n" + "="*70)
    print("102TH STUDY: Blueprint String Framework")
    print("="*70)
