...
def filter_events(events: List[Dict[str, Any]], 
                  event_type: str = None,
                  exclude_types: List[str] = None) -> List[Dict[str, Any]]:
    """
    Filter events by type. Useful for removing rendering artifacts.
    
    Args:
        events: List of event dictionaries
        event_type: If specified, keep only this event type
        exclude_types: If specified, remove these event types
        
    Returns:
        Filtered list of events
        
    Example:
        # Remove barlines from parsed snippet
        notes_only = filter_events(snippet_events, exclude_types=['barline'])
        
        # Keep only notes
        notes_only = filter_events(snippet_events, event_type='note')
    """
    if event_type:
        return [e for e in events if e.get('type') == event_type]
    elif exclude_types:
        return [e for e in events if e.get('type') not in exclude_types]
    else:
...
def transpose_events(events: list, semitones: int) -> list:
    ...
    Automatically filters out barlines to prevent misalignment when transforming
    parsed snippets. Barlines will be re-inserted by the rendering pipeline based
    on the time signature.
    
    Args:
        events: List of event dictionaries (may include barlines from parsing)
        semitones: Number of semitones to transpose (positive = up, negative = down)
    
    Returns:
        New list of transposed events (barlines filtered out)
    """
    transposed = []
    
    for ev in events:
        # Skip barlines - they'll be re-inserted during rendering based on meter
...
def invert_events(events: list, axis_pitch: str = 'c4') -> list:
    ...
    Returns:
        New list of inverted events (barlines filtered out)
    """
    # Parse axis pitch
...
def retrograde_events(events: list) -> list:
    ...
    Automatically filters out barlines to prevent misalignment. Barlines will
    be re-inserted by the rendering pipeline based on the time signature.
    
    Args:
        events: List of event dictionaries (may include barlines from parsing)
    
    Returns:
        New list with reversed event order (barlines filtered out)
    """
    # Filter out barlines before reversing
    notes_only = [ev for ev in events if ev.get('type') != 'barline']
    return list(reversed(notes_only))
