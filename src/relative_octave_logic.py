...
def process_relative_sequence(base_pitch: str, base_octave: int, 
    ...
    for token in tokens:
        # Handle tuplet tokens (passthrough - they contain sub-notes)
        if isinstance(token, dict) and token.get('type') == 'tuplet':
            # Tuplets are processed separately - just pass them through
            # The last note in the tuplet will set the reference for the next note
            results.append((token, 0, False))
            # TODO: Parse tuplet content and update current_midi/current_pitch based on last note
        elif hasattr(token, 'is_tuplet') and token.is_tuplet:
            # ParsedToken with is_tuplet=True
            # Pass through - tuplet notes will be processed later
            results.append((token, 0, False))
            # TODO: Update reference pitch based on last note in tuplet
        elif token.is_rest:
            ...
            # The base note determines the chord's position in the sequence
            ...
            # Return the resolved octave for the base note
            ...
            # Barlines or unparseable tokens (no pitch) - skip but preserve in sequence
            results.append((token, 0, False))
        else:
