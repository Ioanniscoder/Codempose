...
def parse_lilypond_to_data(lily_string: str, part_name: str = 'Part 1', validate_barlines: bool = True) -> dict:
    """
    ...
    Args:
        lily_string: LilyPond source code
        part_name: Name for the part
        validate_barlines: If False, trust all barlines without validation (for MusicXML export)
    """
    ...
    # BUT: For MusicXML export, trust the original barlines
    barline_warnings = []
    if validate_barlines:
        ...
        corrected_tokens = parse_result.tokens
    ...
    # Tie tracking state
    # NEW APPROACH: Don't merge ties - track last note with tie start
    tie_pending_info = None  # Stores pitch info {step, octave, alter} of note waiting for tie
    expect_tie_stop = None  # Stores pitch info after barline (for tie continuation)
    ...
    for token_info in corrected_tokens:
        ...
        if token_info.original == '|':
            # CRITICAL: Handle ties that cross barlines
            # Don't merge - just track that next matching note should be tie='stop'
            if tie_pending_info is not None:
                # Remember pitch for tie continuation after barline
                expect_tie_stop = tie_pending_info.copy()
                tie_pending_info = None
        ...
        if parsed.is_tuplet:
            ...
            # Handle tie notation (NEW: Don't merge, use tie marks for music21)
            # First check if this note is a tie continuation from across a barline
            if expect_tie_stop is not None:
                if (expect_tie_stop['step'] == note_event['step'] and 
                    expect_tie_stop['octave'] == note_event['octave'] and
                    expect_tie_stop['alter'] == note_event['alter']):
                    # This note continues a tie from before the barline
                    note_event['tie'] = 'stop'  # Ties back to previous note
                    expect_tie_stop = None
                    
                    # Check if this note ALSO starts a new tie (tie chain)
                    if parsed.has_tie:
                        note_event['tie'] = 'continue'  # Both stop and start
                        tie_pending_info = {
                            'step': note_event['step'],
                            'octave': note_event['octave'],
                            'alter': note_event['alter']
                        }
                else:
                    # Pitch mismatch - expected tie was broken
                    expect_tie_stop = None
            # Check if this note continues a tie from within same measure
            elif tie_pending_info is not None:
                if (tie_pending_info['step'] == note_event['step'] and 
                    tie_pending_info['octave'] == note_event['octave'] and
                    tie_pending_info['alter'] == note_event['alter']):
                    # This note is a tie continuation within same measure
                    note_event['tie'] = 'stop'  # Ties back to previous note
                    tie_pending_info = None
                    ...
                    # Pitch mismatch - tie broken
                    tie_pending_info = None
            
            # Check if this note starts a tie (and wasn't already handled as continuation)
            if parsed.has_tie and tie_pending_info is None and expect_tie_stop is None:
                # Mark this note as starting a tie
            ...
            # Always append the note (no more merging!)
            events.append(note_event)

    # Note: No need to finalize tie_pending_info - notes are already in events
    ...
    if parse_result.directives:
        ...
        if 'clef' in parse_result.directives:
            # Extract clef (e.g., "bass", "treble", "alto")
            metadata["clef"] = parse_result.directives['clef']
